...
# # Determine if running in PyCharm Console due to differences in running in MobaXterm
isPyCharm = "PYCHARM_HOSTED" in os.environ

## Code added to fix PyCharm bug in helpers
# if isPyCharm:
#     sys.path.append('/S/python/SEDFIT')
#     sys.path.append('/S/python/uga')
#     sys.path.append('/S/SEDFIT/Python3')
...
# # Use plotSED() copied from SED to remove plt.show() command
#  PyCharm SciView causing blank figures with show() comes before savefig()
# def plot_SED():
#     """Using various SED class variables that were filled by other functions (e.g., s.bestfit), create a plot"""
#     fig = plt.figure("SED Plot", figsize=(8,6), dpi=100)
#     sed_ax = fig.add_subplot(111)
#     sed_ax.grid(True, lw=0.1)
#     sed_ax.set_title(label=source_name)
#     sed_ax.set_xlabel(r"$\lambda$ $[\mu m]$", fontsize=15)
#     sed_ax.set_ylabel(r"$F_\nu$ $[Jy]$", fontsize=15)
#     # plot bestfit model
#     sed_ax.loglog(s.star.waves, s.star.fluxes, 'k-', linewidth=0.2, alpha=0.8)
#     # plot disk model
#     # if hasattr(self,'disk'):    # disk_fit was performed
#     #     sed_ax.loglog(s.disk.waves, s.disk.fluxes, 'b:', linewidth=0.2, alpha=0.8)
#     #     sed_ax.loglog(s.disk.waves, s.star.fluxes+s.disk.fluxes, 'k-', linewidth=0.5, alpha=1.0)
#     # now plot observed data
#     pdata = s.inphot
#     upps_pos = (pdata.fl <= pdata.efl) | (pdata.efl <= 0)     # upper limits
#     meas = pdata[~upps_pos]
#     xerror = meas.dwl # if getattr(self,'plot_xerr',False) else None
#     sed_ax.errorbar(meas.wl, meas.fl, xerr=xerror, yerr=meas.efl,
#                     fmt='ko', ms=6, ecolor='0.3', elinewidth=0.2, alpha=0.8)
#     # plot bad photometric data (mostly vizsed data)
#     # if hasattr(self,'vizsed_bidx'):
#     bad_data = s.inphot.loc[s.vizsed_bidx ]
#     sed_ax.plot(bad_data.wl, bad_data.fl, 'rx', ms=12)
#     if any(upps_pos):
#         upps = pdata[upps_pos]
#         sed_ax.errorbar(upps.wl, upps.fl, fmt='rv', ms=8, mec='red', mew=1.2, alpha=0.3)
#     # Mark the wavelength points that were used in the chi2 fitting
#     # ... and set X and Y limits
#     ymin=0.3*np.min(np.abs(pdata.fl))
#     ymax=7.0*np.max(pdata.fl)
#     sed_ax.plot(s.chi2_waves, [ymin]*len(s.chi2_waves), 'r|', ms=20)
#     sed_ax.plot(s.chi2_waves, [ymax]*len(s.chi2_waves), 'r|', ms=20)
#     sed_ax.set_xlim(0.2, 500.0)
#     sed_ax.set_ylim(ymin=ymin, ymax=ymax)
#     sed_ax.set_autoscale_on(False)
#     # fitting information
#     anno_text = r'$T_{star}$ = %.0f' %(sfit_tstar)
#     anno_text += '\n' + r'$apprad$ = %.5f' %(s.sfit_apprad)
#     anno_text += '\n' + r'$\chi^2$ = %.2f' %(s.sfit_chi2)
#     sed_ax.text(0.99, 0.99, anno_text, bbox=dict(facecolor='w', alpha=0.2), linespacing=1.0, \
#                 horizontalalignment='right', verticalalignment='top', transform=sed_ax.transAxes)
#     if isPyCharm:
#         fig.show()
#     fig.savefig('Figures/' + source_name + '_SEDfit')
#     plt.close()
#     return


# Get Observed Data from SED inphot and update source
# def get_inphot():
#     obs_data = s.inphot
#     # Convert vizsed band to string
#     obs_viz = obs_data[obs_data.src == 'vizsed']
#     obs_idx = obs_viz.index
#     obs_viz_band = obs_data.band[obs_idx].str.decode("utf-8", errors="ignore")
#     obs_data.band[obs_idx] = obs_viz_band[obs_idx]
#     return obs_data
...
def get_VO_Data(source_name):
    ...
    return plx, plx_err, teff_dr2, rstar_dr2


# # Function to calculate Rstar value using GAIA or user entered parallax
# def get_Rstar(plx, apprad):
#     if plx == "":
#         g_data = querycat('gaia', position[0], position[1], 3.0)
#         if g_data == None:
#             Rstar = "NaN"
#         else:
#             plx = float(g_data.plx)
#             Rstar = apprad / plx * 1000
#     else:
#         Rstar = apprad / plx * 1000
# #    print "Rstar = ", Rstar
#     return Rstar, plx
...
def plot_sigmas(itr):
    ...
    for i in range(ndim):
        ...
        if i == 1:
            ...
            # Derive, plot and store Stellar Radius Rstar (Rsun) with uncertianties
            # if plx != "":
            #     Rstar_med = store_all['Rstar'] = float('{:.3g}'.format(np.sqrt(scale_med) / plx * 1000))
            #     Rstar_u_neg = store_all['u-(Rstar)'] = float('{:.3g}'.format((np.sqrt((scale_u_neg**2) / (4 * scale_med))
            #                                                   + (scale_med * plx_err**2) / (plx**2)) / plx * 1000))
            #     Rstar_u_plus = store_all['u+(Rstar)'] = float('{:.3g}'.format((np.sqrt((scale_u_plus**2) / (4 * scale_med))
            #                                                   + (scale_med * plx_err**2) / (plx**2)) / plx * 1000))
            #     # if data_bmk_source.empty:
            #     #     Rstar_bmk = store_all['Rstar_bmk'] = round((ang_dia_emc * 1000) / (4.65047 * 2 * plx), 3)
            #     if Rstar_bmk == 'NaN':
            #         Rstar_pct_bmk = store_all['Rstar_delta_bmk'] = 'NaN'
            #     else:
            #         Rstar_pct_bmk = store_all['Rstar_delta_bmk'] = round((100 * (Rstar_med - Rstar_bmk) / Rstar_bmk), 1)
            #     txt = "$\mathrm{{{3}}} = {0:.3g}_{{-{1:.3g}}}^{{+{2:.3g}}}$"
            #     txt = txt.format(Rstar_med, Rstar_u_neg, Rstar_u_plus, 'Rstar')
            #     plt.annotate(txt, xy=(0.05, 0.05), fontsize=9)
            # else:
            #     print("Rstar = NaN")
    plt.xticks([])
...
def lnlike(theta, x, y, y_err):
    ...
    #     check += 1
    #     if check == 1:
    #         print(flux_model, ln_like)
    # # store sampler data and results
    # store_sampler = pd.DataFrame({'ln_like': ln_like}, index=[0])
    # store_sampler['flux_y'] = y
    # store_sampler['flux_model'] = flux_model
    # store_sampler['Teff'] = theta[0]
    # store_sampler['scale'] = theta[1]
    # store_sampler['wl_x'] = x
    # print(store_sampler)
    # store_sampler_all = store_sampler_all.append(store_sampler)
    # store_test = store_sampler_all
    # store_sampler_all.to_csv('Sampler/' + sname + '-' + starmodel + '-' + stored + '_data.csv', index=False)
    # print(store_sampler_all)
#    return -0.5 * np.sum(((y - model(theta, x)) / y_err) ** 2)   # Chi-Squared function
    return ln_like
    # return -0.5 * np.sum(((y - model(theta, x))**2 / model(theta, x)))  # Pearson Chi-Squared statistic
#    return -0.5 * np.sum(((y - model(theta, x))**2 / y_err))
...
# # Set SED3() Observed Photometry Parameters
#s.use_all_vizsed_data = True
#s.vizsed_in_fit = True
#s.plot_vizsed_data = True
...
# if USE_SED:
#     if not sposition:
#         # s.fit(source_name)     # SED() method to fit star using source name. Includes plot and summary.
#         print(source_name)
#         s.source_name = sname
#         s.get_data()           # SED() method to get observed data using position
#         s.fit_star()           # SED() method to fit star
#         s.summarize()          # Print summary of SED() results.
#     else:
#         # # Fit Star using position
#         print(position)
#         s.get_data()           # SED() method to get observed data using position
#         s.fit_star()           # SED() method to fit star
#         s.summarize()          # Print summary of SED() results.
...
plx, plx_err, teff_dr2, rstar_dr2 = get_VO_Data(source_name)
...
##  Temporary Code to Fix Teff at a single value
# min_model_temp = Teff_bmk
# max_model_temp = Teff_bmk
...
# # Get parallax with uncertianty and other VO Data
#plx, plx_err = get_VO_Data(source_name)
...
#   # Store results
#   store_all['RA'] = position[0]
#   store_all['DEC'] = position[1]
#   store_all  ['Model'] = starmodel

# if USE_SED:
#     # Define SEDFit Temp Tstar & % Delta Benchmark
#     sfit_tstar = store_all['Tstar'] = int(sfit_tstar)
#     if Teff_bmk == "NaN":
#         sfit_tstar_pct_bmk = store_all['Tstar_delta_bmk'] = "NaN"
#     else:
#         sfit_tstar_pct_bmk = store_all['Tstar_delta_bmk'] = "{:.1f}".format((100 * (sfit_tstar - Teff_bmk) / Teff_bmk))
    
#     # Define SEDFit Apparent Radius
#     sfit_apprad = store_all['Apprad_sfit'] = float('{:.3g}'.format(s.sfit_apprad))
    
#     # Define SEDFit Angular Diameter & % Delta Benchmark
#     sfit_ang_dia = store_all['Ang_Dia_sfit']  = float('{:.3g}'.format((sfit_apprad * 4.65047 * 2)))
#     if ang_dia_bmk == "NaN":
#         sfit_ang_dia_pct_bmk = store_all['Ang_Dia_sfit_delta_bmk'] = "NaN"
#     else:
#         sfit_ang_dia_pct_bmk = store_all['Ang_Dia_sfit_delta_bmk'] = "{:.1f}".format((100 * (sfit_ang_dia - ang_dia_bmk) / ang_dia_bmk))
    
#     # Define SEDFit Radius Rstar & % Delta Benchmark
#     sfit_rstar = store_all['Rstar_sfit'] = float('{:.3g}'.format((sfit_apprad / plx * 1000)))
#     if Rstar_bmk == "NaN":
#         sfit_rstar_pct_bmk = store_all['Rstar_sfit_delta_bmk'] = "NaN"
#     else:
#         sfit_rstar_pct_bmk = store_all['Rstar_sfit_delta_bmk'] = "{:.1f}".format((100 * (sfit_rstar - Rstar_bmk) / Rstar_bmk))
#     chi2 = store_all['Min Chi2'] = s.sfit_chi2
#     sfit_pts = store_all['Fit Points_sfit'] = len(s.chi2_waves)
...
# # Use SED3() to get Photometry data
# obs_data = get_inphot()
# obs_data = filter_bands(obs_data)
# obs_data = remove_dups(obs_data)
# obs_data_range = filter_wl(obs_data, wl_min, wl_max)
...
# if USE_SED:
#     # Limit range of SEDfit curve
#     SED_fit = s.star[(wl_min <= s.star.waves) & (s.star.waves <= wl_max)]
...
#         ["SED Fit Results {delta bmk}", str(sfit_tstar) + '  {' + str(sfit_tstar_pct_bmk) + '%}', str(sfit_rstar) + '  {' + str(sfit_rstar_pct_bmk) + '%}',
#             str(sfit_ang_dia) + '  {' + str(sfit_ang_dia_pct_bmk) + '%}', ],  # 'testing','size'],
...
while loop:
    ...
    if edit_pts == "y\n":
        ...
        # Reading and show png image file for XTerm sessions
        if not isPyCharm:
            im1 = Image.open('Figures/' + source_name + '_Observed_' + str(itr) + '.png')
            im2 = Image.open('Figures/' + source_name + '_Observed_Table' + str(itr) + '.png')
            # show images
            im1.show()
            im2.show()
        # Pick New Bad Points
        ...
        # Calculate Uncertainties & Plot
        teff_med, teff_u_neg, teff_u_plus, teff_med_pct_bmk, scale_med, scale_u_neg, scale_u_plus, \
        apprad_med, apprad_u_neg, apprad_u_plus, ang_dia_emc, ang_dia_emc_u_neg, ang_dia_emc_u_plus, \
        ang_dia_emc_pct_bmk, Rstar_med, Rstar_u_neg, Rstar_u_plus = plot_sigmas(itr)
...
##  flat_all_df.to_csv('Sampler/' + starmodel + '/' + sname + '-' + starmodel + '-' + stored + '_chain.csv', index=False)
# store_sampler_all.to_csv('Sampler/' + sname + '-' + starmodel + '-' + stored + '_data.csv', index=False)
# sampler.chain.to_csv('Sampler/' + sname + '_chain.csv', index=False)
# sampler.flatchain.to_csv('Sampler/' + sname + '_flatchain.csv', index=False)
# sampler.lnprobability.to_csv('Sampler/' + sname + '_lnprobability.csv', index=False)
# sampler.flatlnprobability.to_csv('Sampler/' + sname + '_flatlnprobability.csv', index=False)